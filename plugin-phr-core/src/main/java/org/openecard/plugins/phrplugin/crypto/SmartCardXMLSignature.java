...
/**
 * Handles <code>&lt;ds:Signature&gt;</code> elements. This is the main class that deals with creating and verifying
 * signatures.
 * 
 * <p>
 * There are 2 types of constructors for this class. The ones that take a document, baseURI and 1 or more Java Objects.
 * This is mostly used for signing purposes. The other constructor is the one that takes a DOM Element and a baseURI.
 * This is used mostly with for verifying, when you have a SignatureElement.
 * 
 * There are a few different types of methods:
 * <ul>
 * <li>The addDocument* methods are used to add References with optional transforms during signing.</li>
 * <li>addKeyInfo* methods are to add Certificates and Keys to the KeyInfo tags during signing.</li>
 * <li>appendObject allows a user to add any XML Structure as an ObjectContainer during signing.</li>
 * <li>sign and checkSignatureValue methods are used to sign and validate the signature.</li>
 * </ul>
 * 
 * <b>Note, that the sign method was modified by Dirk Petrautzki <dirk.petrautzki@hs-coburg.de> 
 * to use {@link GenericCryptoSigner} for signing.</b> 
 */
public final class SmartCardXMLSignature extends SignatureElementProxy {
    ...
    private static org.apache.commons.logging.Log log = org.apache.commons.logging.LogFactory
	    .getLog(SmartCardXMLSignature.class);
    ...
    /**
     * Checking the digests in References in a Signature are mandatory, but for References inside a Manifest it is
     * application specific. This boolean is to indicate that the References inside Manifests should be validated.
     */
    ...
    /**
     * This creates a new <CODE>ds:Signature</CODE> Element and adds an empty <CODE>ds:SignedInfo</CODE>. The
     * <code>ds:SignedInfo</code> is initialized with the specified Signature algorithm and
     * Canonicalizer.ALGO_ID_C14N_OMIT_COMMENTS which is REQUIRED by the spec. This method's main use is for creating a
     * new signature.
     * 
     * @param doc
     *            Document in which the signature will be appended after creation.
     * @param baseURI
     ...
     */
    public SmartCardXMLSignature(Document doc, String baseURI, String signatureMethodURI)
	    throws XMLSecurityException {
     ...
     * @throws XMLSecurityException
     */
    public SmartCardXMLSignature(Document doc, String baseURI, String signatureMethodURI, int hmacOutputLength)
	    throws XMLSecurityException {
     ...
     *            the canonicalization algorithm to be used to c14nize the SignedInfo element.
     * @throws XMLSecurityException
     */
    public SmartCardXMLSignature(Document doc, String baseURI, String signatureMethodURI,
	...
	if (xmlnsDsPrefix == null || xmlnsDsPrefix.length() == 0) {
	    this.constructionElement.setAttributeNS(Constants.NamespaceSpecNS, "xmlns", Constants.SignatureSpecNS);
	} else {
	...
	signatureValueElement = XMLUtils.createElementInSignatureSpace(this.doc, Constants._TAG_SIGNATUREVALUE);
     ...
     * @throws XMLSecurityException
     */
    public SmartCardXMLSignature(Document doc, String baseURI, Element SignatureMethodElem,
     ...
     */
    public SmartCardXMLSignature(Element element, String baseURI) throws XMLSignatureException,
     ...
     *             if the signature is badly formatted
     */
    public SmartCardXMLSignature(Element element, String baseURI, boolean secureValidation)
    ...
    /**
     * Returns the octet value of the SignatureValue element. Throws an XMLSignatureException if it has no or wrong
     * content.
     * 
    ...
    /**
     * Appends an Object (not a <code>java.lang.Object</code> but an Object element) to the Signature. Please note that
     * this is only possible when signing.
     * 
     * @param object
    ...
    /**
     * Returns the <code>i<code>th <code>ds:Object</code> child of the signature or null if no such
    ...
    /**
     * Digests all References in the SignedInfo, calculates the signature value and sets it in the SignatureValue
     * Element.
     * 
     * @param signer the {@link GenericCryptoSigner} that is used to sign.
     * @throws XMLSignatureException
     */
    public void sign(GenericCryptoSigner signer) throws XMLSignatureException {

	try {
	    SignedInfo si = this.getSignedInfo();
	    // TODO check SignatureAlgorithm information 
		...
		so = new UnsyncBufferedOutputStream(baos);
	    ...
	    this.setSignatureValueElement(val);
    ...
    /**
     * Extracts the public key from the certificate and verifies if the signature is valid by re-digesting all
     * References, comparing those against the stored DigestValues and then checking to see if the Signatures match on
     * the SignedInfo.
     * 
     * @param cert
     *            Certificate that contains the public key part of the keypair that was used to sign.
    ...
    /**
     * Verifies if the signature is valid by redigesting all References, comparing those against the stored DigestValues
     * and then checking to see if the Signatures match on the SignedInfo.
     * 
     * @param pk
     *            {@link java.security.PublicKey} part of the keypair or {@link javax.crypto.SecretKey} that was used to
     *            sign
     * @return true if the signature is valid, false otherwise
     * @throws XMLSignatureException
     */
	...
	try {
	    ...
	    try {
		m = SignedInfo.class.getDeclaredMethod("getSignatureAlgorithm", null);
	    ...
	    try {
		sa = (SignatureAlgorithm) m.invoke(si, new Object[0]);
     ...
     * 
     * @param referenceURI
     *            URI of the resource to be signed. Can be null in which case the dereferencing is application specific.
     *            Can be "" in which it's the parent node (or parent document?). There can only be one "" in each
     *            signature.
     * @param trans
    ...
    /**
     * Adds a Reference with just the URI and the transforms. This used the SHA1 algorithm as a default digest
     * algorithm.
     * 
     * @param referenceURI
     *            URI according to the XML Signature specification.
     ...
     * 
     * @param cert
     *            Certificate to be included. This should be the certificate of the key that was used to sign.
     * @throws XMLSecurityException
     */
    ...
    /**
     * Proxy method for {@link SignedInfo#createSecretKey(byte[])}. If you want to create a MAC, this method helps you
     * to obtain the {@link javax.crypto.SecretKey} from octets.
     * 
    ...
    /**
     * Signal wether Manifest should be automatically validated. Checking the digests in References in a Signature are
     * mandatory, but for References inside a Manifest it is application specific. This boolean is to indicate that the
     * References inside Manifests should be validated.
     * 
     * @param followManifests
     * @see <a href="http://www.w3.org/TR/xmldsig-core/#sec-CoreValidation"> Core validation section in the XML
     *      Signature Rec.</a>
     */
